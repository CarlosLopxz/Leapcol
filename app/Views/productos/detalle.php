...
<div class="container mt-5 mb-5">
    ...
    <div class="position-relative">
        <!-- Navigation Buttons -->
        <button class="btn btn-white shadow rounded-circle position-absolute top-50 start-0 translate-middle-y z-2 d-flex align-items-center justify-content-center"
        ...
        <button class="btn btn-white shadow rounded-circle position-absolute top-50 end-0 translate-middle-y z-2 d-flex align-items-center justify-content-center"
        ...
        <!-- Carousel Container -->
        <div id="static-related-products" class="d-flex gap-4 py-3 px-1 overflow-auto" style="scroll-behavior: smooth; scrollbar-width: none;">
                ...
                <?php foreach ($relacionados as $relacionado): ?>
                    <div class="card shadow-sm border-0 flex-shrink-0 rounded-4 overflow-hidden" style="width: 300px;">
                        <div class="position-relative" style="height: 200px; overflow: hidden;">
                            <div class="d-flex align-items-center justify-content-center h-100 bg-light text-secondary">
                                <?php if (!empty($relacionado['imagen_principal'])): ?>
                                    <img src="<?= base_url('assets/img/productos/' . $relacionado['imagen_principal']) ?>" class="w-100 h-100 object-fit-cover" alt="<?= esc($relacionado['nombre']) ?>">
                                <?php else: ?>
                        ...
                        <div class="card-body d-flex flex-column p-3">
                            <div class="text-danger fw-bold mb-1 text-uppercase" style="font-size: 0.65rem;"><?= esc($relacionado['categoria_nombre']) ?></div>
                            <h5 class="card-title fw-bold text-dark mb-2" style="font-size: 1rem;"><?= esc($relacionado['nombre']) ?></h5>
                            <p class="text-muted mb-3" style="font-size: 0.75rem; line-height: 1.4; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden;"><?= esc($relacionado['descripcion']) ?></p>
                            
                            <div class="d-flex gap-2 mb-3">
                                <span class="badge bg-light text-dark border fw-normal" style="font-size: 0.7rem;">Stock: <?= $relacionado['stock'] ?></span>
                            ...
                            <div class="d-flex align-items-end gap-2 mb-3">
                                <?php if ($relacionado['precio_oferta'] > 0): ?>
                                    <span class="text-decoration-line-through text-muted fw-bold opacity-50" style="font-size: 0.85rem;">$<?= number_format($relacionado['precio'], 0, ',', '.') ?></span>
                                    <span class="fw-bold text-dark lh-1" style="font-size: 1.25rem;">$<?= number_format($relacionado['precio_oferta'], 0, ',', '.') ?></span>
                                <?php else: ?>
                                    <span class="fw-bold text-dark lh-1" style="font-size: 1.25rem;">$<?= number_format($relacionado['precio'], 0, ',', '.') ?></span>
                                <?php endif; ?>
                            </div>

                            <div class="mt-auto">
                                <a href="<?= base_url('productos/detalle/' . $relacionado['id']) ?>" class="btn btn-danger w-100 fw-bold py-2" style="font-size: 0.85rem;">Ver Producto</a>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
